...
async function loadDailyActivity() {
    ...
    try {
                const [auditsSnap, movsSnap] = await Promise.all([
                                firebase.database().ref(`auditorias/${determinante}`).once('value'),
                                firebase.database().ref(`movimientos/${determinante}`).once('value')
                            ]);

            const audits = Object.values(auditsSnap.val() || {})
              .filter(a => a.fecha && isoToLocalDate(a.fecha) === today);
                const movs = Object.values(movsSnap.val() || {})
              .filter(m => m.fecha && isoToLocalDate(m.fecha) === today && (m.tipo === 'salida' || m.tipo === 'entrada_directa_anaquel'));

            // Calcular totales
            let totalProductosAuditados = 0;
                audits.forEach(a => totalProductosAuditados += (a.productos ? Object.keys(a.productos).length : 0));

            let totalCajasMovidas = 0;
                movs.forEach(m => totalCajasMovidas += (parseFloat(m.cajasMovidas) || 0));

            window.PROFILE_STATE.todayActivity = {
                            auditorias: audits.length,
                            productosAuditados: totalProductosAuditados,
                            rellenos: movs.length,
